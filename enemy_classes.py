...
class Enemy(entity_classes.DamagableAttacker):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        ...
        self.health_bar_texture = arcade.load_texture("enemy health bar.png")
        self.health_bar_backing = arcade.Sprite("red pixel.png", 1)
        ...
        self.health_bar_filling = arcade.Sprite("green pixel.png", 1)
        ...
        self.health_bar = arcade.Sprite("enemy health bar.png", ENEMY_SCALE)
        ...
        elif self.top > self.window.height:
...
class Gomba(WalkingEnemy):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("gomba.png", mirrored=True), arcade.load_texture("gomba.png")]
        self.set_texture(1)
...
class Romba(Gomba):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        ...
        self.textures = [arcade.load_texture("romba.png", mirrored=True), arcade.load_texture("romba.png"), arcade.load_texture("romba falling.png")]
        self.set_texture(1)
...
class Bomba(Gomba):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("bomba.png", mirrored=True), arcade.load_texture("bomba.png")]
        self.set_texture(1)
        self.shaking = False
        self.die_x = None
        self.die_y = None
    ...
    def explode(self):
        ...
        explosion.textures = [arcade.load_texture("bomba explosion.png")]
...
class Jomba(Gomba):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        ...
        self.textures = [arcade.load_texture("jomba.png", mirrored=True), arcade.load_texture("jomba.png")]
        self.set_texture(1)
...
class Vomba(Gomba):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength, launch_power, bounce_mult, proj_gravity, air_resist):
        ...
        self.textures = [arcade.load_texture("vomba.png", mirrored=True), arcade.load_texture("vomba.png")]
        self.set_texture(1)
...
class FireBall(projectile_classes.DamagingProjectile):
    def setup(self, launcher, window, id, boundary_type, target_list, damage, frame_delay, kb_x, kb_y, gravity, friction, strength):
        ...
        self.textures = [arcade.load_texture("vomba projectile.png")]
        self.set_texture(0)
...
class IBall(Enemy):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("iball.png"), arcade.load_texture("iball scared.png"), arcade.load_texture("iball scared.png", mirrored=True)]
        self.set_texture(0)
    ...
    def update(self):
        if not self.is_dead:
            ...
            self.alpha -= 255/ENEMY_DEAD_FRAMES
...
class MBall(IBall):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("mball.png", mirrored=True), arcade.load_texture("mball.png")]
        self.set_texture(1)
...
class BBall(IBall):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("bball.png")]
        self.set_texture(0)
        self.direction = 0
        self.x_move = 0
        self.y_move = 0
...
class FallingBBall(entity_classes.AttackerSprite):
    def setup(self, target_list, damage, frame_delay, kb_x, kb_y, gravity, friction, strength):
        ...
        self.textures = [arcade.load_texture("bball falling.png")]
        self.set_texture(0)
        self.scale = ENEMY_PROJECTILE_SCALE
...
class CBall(IBall):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("cball.png"), arcade.load_texture("cball happy.png")]
        self.set_texture(0)
...
class EBall(IBall):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        ...
        self.textures = []
        for i in range(1, 9):
            self.textures.append(arcade.load_texture(str(i) + "ball.png"))
        self.set_texture(7)
        self.direction = 0
...
class SDude(WalkingEnemy):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("sword man.png", mirrored=True), arcade.load_texture("sword man.png")]
        self.set_texture(1)
        ...
        self.weapon.textures = [arcade.load_texture("sword man sword.png")]
...
class ADude(SDude):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("axe man.png", mirrored=True), arcade.load_texture("axe man.png")]
        self.set_texture(1)
        self.weapon = equipment_classes.Weapon()
        self.weapon.setup(target_list, self.damage, self.frame_delay, self.kb_x, self.kb_y, 0, 0, self.strength, self.window, self)
        self.window.enemy_projectile_list.append(self.weapon)
        self.weapon.textures = [arcade.load_texture("axe man axe.png")]
        self.weapon.set_texture(0)
        self.target_list = None
...
class BDude(SDude):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength, launch_power, bounce_mult, proj_gravity, air_resist):
        ...
        self.textures = [arcade.load_texture("bow man.png", mirrored=True), arcade.load_texture("bow man.png")]
        self.set_texture(1)
        self.weapon = arcade.Sprite()
        self.window.enemy_projectile_list.append(self.weapon)
        self.weapon.textures = [arcade.load_texture("bow man bow.png", mirrored=True), arcade.load_texture("bow man bow.png")]
...
class Dart(projectile_classes.DamagingProjectile):
    def setup(self, launcher, window, id, boundary_type, target_list, damage, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(launcher, window, id, boundary_type, target_list, damage, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("bow man arrow.png")]
        self.set_texture(0)
        self.scale = ENEMY_PROJECTILE_SCALE
...
class DDude(SDude):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("donut man.png", mirrored=True), arcade.load_texture("donut man.png")]
        self.set_texture(1)
        self.weapon = equipment_classes.Weapon()
        self.weapon.setup(None, self.damage, self.frame_delay, self.kb_x, self.kb_y, 0, 0, self.strength, self.window, self)
        self.window.enemy_projectile_list.append(self.weapon)
        self.weapon.textures = [arcade.load_texture("donut man donut.png"), arcade.load_texture("donut man donut eaten.png"), arcade.load_texture("donut man donut eaten more.png")]
        self.weapon.set_texture(0)
        self.target_list = None
        self.bite_num = 0
...
class HDude(SDude):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        super().setup(target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength)
        self.textures = [arcade.load_texture("hammer man.png", mirrored=True), arcade.load_texture("hammer man.png")]
        self.set_texture(1)
        self.weapon = equipment_classes.Weapon()
        self.weapon.setup(target_list, self.damage, self.frame_delay, self.kb_x, self.kb_y, 0, 0, self.strength, self.window, self)
        self.window.enemy_projectile_list.append(self.weapon)
        self.weapon.textures = [arcade.load_texture("hammer man hammer.png")]
        self.weapon.set_texture(0)
        self.target_list = None
        self.stuck = False
        self.lifting = False
