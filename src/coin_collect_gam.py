...
class Coin(arcade.Sprite):
    def setup(self):
        ...
        self.textures.append(arcade.load_texture(f"assets\{str(random.randint(1, 9))}coin.png"))
        self.set_texture(0)
...
class Consumable(Coin):
    def setup(self):
        ...
        self.textures = [arcade.load_texture(r"assets\egg.png"), arcade.load_texture(r"assets\apple.png"), arcade.load_texture(r"assets\bread.png"), arcade.load_texture(r"assets\chip bag.png"), arcade.load_texture(r"assets\steak.png")]
...
class Game(arcade.Window):
    ...
    def setup(self):
        ...
        #set window vars
        self.background = arcade.load_texture(r"assets\background.png")
        self.set_mouse_visible(False)
        self.cursor_image = arcade.Sprite(r"assets\mouse cursor.png", MOUSE_SCALE)
        ...
        health_bar = arcade.Sprite(r"assets\health bar.png", HEALTH_BAR_SCALE)
        health_bar_backing = arcade.Sprite(r"assets\red pixel.png", 1)
        ...
        health_bar_filling = arcade.Sprite(r"assets\green pixel.png", 1)
        ...
        health_bar_gui = arcade.Sprite(r"assets\health bar.png", HEALTH_BAR_SCALE)
        ...
        self.grappling_hook_launcher = projectile_classes.Launcher(r"assets\grapple launcher.png", LAUNCHER_SCALE)
        ...
        self.bow = projectile_classes.Launcher(r"assets\bow.png", LAUNCHER_SCALE)
        ...
        shop_button = button_classes.Button(r"assets\shop icon.png", ICON_SCALE)
        ...
        exit_shop_button = button_classes.Button(r"assets\x icon.png", ICON_SCALE)
        ...
        instructions = arcade.Sprite(r"assets\instructions.png", 1)
        instructions.set_position(SCREEN_WIDTH/2, SCREEN_HEIGHT/2)
        self.pause_screen_list.append(instructions)

        stats_box = arcade.Sprite(r"assets\statistics box.png", 1)
        stats_box.left = 625
    ...
    def on_update(self, delta_time: float):
        ...
        if not self.in_shop and not self.lost:
            ...
            if self.splash_list[0].alpha > 0:
                new_alpha = max(0, self.splash_list[0].alpha - 255/self.wave_delay)
                self.splash_list[0].alpha = new_alpha
            ...
            if self.frame > (self.load_screen_duration + 60):
                if self.frame == self.next_coin_spawn_frame and len(self.coin_list) < self.max_coins:
                    ...
                    coin = Coin(f"assets\{str(random.randint(1, 9))}coin.png", COIN_SCALE)
                    coin.setup()
        ...
        if self.cutscene:
            ...
            if self.real_frame > self.load_screen_duration:
                new_alpha = 255 - (self.real_frame - self.load_screen_duration) * _fade_intensity
                if new_alpha <= 0:
                    self.cutscene = False
                    self.pause_screen_list[0].alpha = 0
                    self.pause_screen_list[1].alpha = 0
                else:
                    self.pause_screen_list[0].alpha = new_alpha
                    self.pause_screen_list[1].alpha = new_alpha
