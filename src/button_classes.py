...
class UpgradeImg(arcade.Sprite):
    def setup(self, window):
        self.textures = arcade.load_spritesheet(r"assets\icon sheet.png", 100, 100, 6, 6 * 6)
        self.window = window
...
class ShopButton(Button):
    def setup(self, window, x, y):
        ...
        self.textures = [arcade.load_texture(r"assets\buy button.png"), arcade.load_texture(r"assets\dont buy button.png")]
        self.set_texture(0)
        self.scale = ICON_SCALE

        self.upgrade_image = UpgradeImg(r"assets\red pixel.png", 1)
        self.upgrade_image.setup(window)
        self.upgrade_image.set_position(x, y)
        self.window.image_list.append(self.upgrade_image)

        self.cost_box = arcade.Sprite(r"assets\cost box.png", ICON_SCALE)
