...
class Grapple(projectile_classes.Projectile):
    def setup(self, launcher, window, id, boundary_type):
        ...
        self.textures = [arcade.load_texture(r"assets\grapple.png")]
        self.set_texture(0)
...
class Arrow(projectile_classes.DamagingProjectile):
    def setup(self, launcher, window, id, boundary_type, target_list, frame_delay, kb_x, kb_y, gravity, friction, strength):
        ...
        self.textures = [arcade.load_texture(r"assets\arrow.png")]
        self.set_texture(0)
    ...
    def when_retracting(self):
        if not self.grounded:
            self.bounce()
        if self.grounded:
            self.alpha = max(0, self.alpha - 255/ARROW_LIFE_SPAN)
...
class Sword(Weapon):
    def setup(self, target_list, damage, frame_delay, kb_x, kb_y, gravity, friction, strength, window, owner):
        ...
        self.textures = [arcade.load_texture(r"assets\sword.png")]
        self.set_texture(0)
...
class Hat(Clothes):
    def setup(self, body_part, x_just, y_just, window):
        ...
        self.textures = [arcade.load_texture(r"assets\helmet.png", mirrored=True), arcade.load_texture(r"assets\helmet.png")]
        self.set_texture(1)
...
class Shoe(Clothes):
    def setup(self, body_part, x_just, y_just, window):
        super().setup(body_part, x_just, y_just, window)
        self.textures = [arcade.load_texture(r"assets\shoe.png"), arcade.load_texture(r"assets\shoe.png", flipped=True)]
        self.set_texture(0)
...
class Crab(enemy_classes.WalkingEnemy):
    def setup(self, target_list, body_sprite, window, damage, hp, defense, speed, worth, frame_delay, kb_x, kb_y, gravity, friction, strength):
        ...
        self.textures = [arcade.load_texture(r"assets\grab crab.png", mirrored=True), arcade.load_texture(r"assets\grab crab.png")]
        self.set_texture(1)
